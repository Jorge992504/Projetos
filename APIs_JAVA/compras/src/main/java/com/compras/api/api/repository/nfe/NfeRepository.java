...
public interface NfeRepository extends JpaRepository<Nfe, Long> {
    ...
    @Query("SELECT new com.compras.api.api.dto.response.ResponseGastosItemDto(n.descricao, n.unit, n.qtde, n.un, n.vlTotal) " +
            "FROM Nfe n " +
            "WHERE n.user.id = :userId " +
            "AND EXTRACT(YEAR FROM n.dataTime) = :ano " +
            "AND EXTRACT(MONTH FROM n.dataTime) = :mes " +
            "ORDER BY n.dataTime ASC")
    List<ResponseGastosItemDto> findByUserAndMesAno(
